...
class MultiValidator(multiTemplate: MultiTemplate) {
  ...
  private def resolveRef(name: String, fromPackage: String): Option[TemplateDefinition] =
    if (name.contains('.')) multiTemplate.definitions.find(d => multiTemplate.fqn(d) == name)
      ...
      candidates
        .find(_.fullPackage(basePackage) == fromPackage)
  ...
  /**
   * Whether the phase-two custom-validator traversal does anything for `definitionName` — i.e. its reachable subtree
   * declares any validators. Package-private for tests.
   */
   ...
   *   The name of the definition to validate against
   * @return
   *   Either a list of validation errors or the parsed JSON
   */
   ...
   *
   * Validation runs in two phases. Phase one checks structure (types, constraints, references); this recursion is
  ...
  private def collectValidatorErrors(
    ...
    templateType match {
        ...
        json.asObject match {
          case Some(obj) =>
            ...
            val extraErrors =
              additional match {
                case TypedExtra(t) =>
                  obj.keys.filterNot(fields.contains).toList.flatMap { k =>
                    obj
                      .field(k)
                      .toList
                      .flatMap(v => collectValidatorErrors(t, v, root, s"$path.$k", currentPackage, depth + 1))
                  }
                case _ => Nil
              }
      ...
      case ArrayType(elementType, _) =>
        json.asArray match {
          case Some(array) =>
            array.values.toList.zipWithIndex.flatMap { case (element, idx) =>
      ...
      case UnionType(types) =>
        ...
        types
          .find(t => validateTypeWithRefs(t, json, path, currentPackage, depth + 1).isEmpty)
          .toList
          .flatMap(t => collectValidatorErrors(t, json, root, path, currentPackage, depth + 1))
      case TypeDiscriminator(fieldName, variants, commonFields, _, _, variantMatch, _, _, _) =>
        collectDiscriminatorValidatorErrors(
          fieldName,
          variants,
          commonFields,
          json,
    ...
    resolveRef(typeName, currentPackage) match {
      case Some(refDef) =>
        ...
        here ++ collectValidatorErrors(
          ...
          root,
          path,
          refDef.fullPackage(basePackage),
          depth + 1)
  ...
  private def collectDiscriminatorValidatorErrors(
    ...
    json.asObject.flatMap(obj => obj.field(fieldName).flatMap(_.asString)).toList.flatMap { discriminatorValue =>
      ...
      matched.toList.flatMap { variantType =>
        ...
        val commonErrors =
          commonFields.toList.flatMap { case (cfName, cfDef) =>
            obj
              .field(cfName)
              .filterNot(_ == JsonNull)
              .toList
              .flatMap(v =>
                collectValidatorErrors(cfDef.fieldType, v, root, s"$path.$cfName", currentPackage, depth + 1))
          }
        val variantErrors =
          variantType.fields.toList.flatMap { case (vfName, vfDef) =>
            obj
              .field(vfName)
              .filterNot(_ == JsonNull)
              .toList
              .flatMap(v =>
                collectValidatorErrors(vfDef.fieldType, v, root, s"$path.$vfName", currentPackage, depth + 1))
          }
  ...
  private def validateTypeWithRefs(
    ...
    templateType match {
      ...
      case ArrayType(elementType, constraints) =>
        validateArray(elementType, json, path, currentPackage, depth, constraints)
      ...
      case EnumType(_, values) =>
        json.asString match {
          ...
          case None =>
            List(ValidationError.constraintViolation(path, s"enum: ${values.mkString(", ")}", jsonType(json)))
        }
    }
  }
  ...
  private def validateWholeNumber(
      json: JsonValue,
      path: String,
      expected: String,
  ...
  private def validateNumberConstraints(
      n: JsonNumber,
      path: String,
      constraints: List[Constraint]): List[ValidationError] =
  ...
  private def validateTemporal(
      json: JsonValue,
      path: String,
      expected: String,
      parse: String => Any): List[ValidationError] =
    json.asString match {
      case None =>
        List(ValidationError.typeMismatch(path, expected, jsonType(json)))
      case Some(value) =>
  ...
  private def validateObject(
    ...
    json.asObject match {
      case Some(obj) =>
        ...
        val fieldErrors =
          fields.flatMap { case (fieldName, fieldDef) =>
        ...
        val extraFields =
          additional match {
            ...
            case TypedExtra(t) =>
              extraKeys.flatMap(k => validateTypeWithRefs(t, jsonFieldMap(k), s"$path.$k", currentPackage, depth + 1))
            case ForbidExtra =>
              extraKeys.map { extraField =>
                ValidationError(
                  s"$path.$extraField",
          ...
          case Some(discriminatorNode) if discriminatorNode.isString =>
            ...
            matched match {
              case Some(variantType) =>
                ...
                val variantErrors =
                  variantType.fields.flatMap { case (vfName, vfDef) =>
                ...
                val extraFields =
                  jsonFieldMap.keySet.diff(allowedFields).toList.map { extraField =>
                    ValidationError(
                      s"$path.$extraField",
                      s"Unexpected field '$extraField' not defined in template",
