 ...
 *
 * @param name
 *   The name of the type to generate (must be a valid Scala identifier)
 * @param templateType
 *   The type definition
 * @param subPackage
 *   Optional sub-package relative to the base package
 * @param description
 *   Optional description of this definition
 * @param validators
 *   Names of registered custom validators to run after schema validation
 * @param sourcePath
 *   Generation-time pointer to the template file this definition came from
 */
 ...
 *
 * @param basePackage
 *   Base package for all generated code (derived from template path)
 * @param definitions
...
case class MultiTemplate(
      ...
      definitions.foreach { definition =>
        ...
        refs.foreach { refName =>
          ...
          if (candidates.isEmpty) {
            ...
            errors =
              s"Definition '${definition.name}' references ambiguous type '$refName' " +
  ...
  /**
   * Collects every simple type-reference name used within a template type (ReferenceType and RecursiveRef), so
   * build-time validation can flag an unresolved or ambiguous reference.
   */
  private def collectReferenceNames(templateType: TemplateType): Set[String] = {
    def findInType(tt: TemplateType): Set[String] =
      tt match {
...
object MultiTemplate {
  ...
  /**
   * Array collection types the generator can emit a matching decoder for. Field types honor `listType`, but the decoder
   ...
   * `java.util.Map` for a Java-interop-friendly public surface.
   */
   ...
   */
  def single(basePackage: String, definition: TemplateDefinition): MultiTemplate =
   ...
   * applies to the whole project. A setting given conflicting non-default values in different files is a build error,
   * so the merged result is unambiguous regardless of file-discovery order.
   */
  def combine(templates: List[MultiTemplate]): Either[String, MultiTemplate] = {
    val absolute =
      templates.flatMap(t => t.definitions.map(d => d.copy(subPackage = Some(d.fullPackage(t.basePackage)))))
    ...
    val conflicts =
      List(listType, fromJsonStyle, dateType, dateTimeType, mapType, visibility)
        .collect { case Left(msg) => msg }
    if (conflicts.nonEmpty)
      Left(
        "Conflicting project-wide settings across template files: " + conflicts.mkString("; ") +
      ...
      Right(
        MultiTemplate(
          ...
          defs,
          listType.right.get,
          fromJsonStyle.right.get,
          dateType.right.get,
          dateTimeType.right.get,
          mapType.right.get,
  ...
  private def commonPrefix(pkgs: List[String]): String =
    if (pkgs.isEmpty) ""
      ...
      pkgs
        .map(_.split('.').toList)
        .reduce((a, b) => a.zip(b).takeWhile { case (x, y) => x == y }.map(_._1))
        .mkString(".")
}
