...
/**
 * The validator recurses over the template with a depth budget so an adversarial/cyclic template
 * (e.g. a reference cycle that consumes no payload) fails with an error instead of a
 * StackOverflowError, and constraint regexes are compiled once and reused.
 */
class ValidatorLimitsSpec extends AnyFlatSpec with Matchers {
  ...
  "MultiValidator" should "fail with an error instead of StackOverflow on a reference cycle" in {
    val t = MultiTemplate("com.example", List(
      TemplateDefinition("A", ReferenceType("B")),
      TemplateDefinition("B", ReferenceType("A"))))
  ...
  "pattern validation" should "accept matching and reject non-matching values (compiled once, reused)" in {
    val t = MultiTemplate("com.example", List(TemplateDefinition("N", ObjectType(ListMap(
      "code" -> FieldDef(StringType(List(Pattern("^[0-9]{3}$"))), optional = false))))))
