...
/**
 * Same simple name in different sub-packages is supported, so runtime `$ref` resolution must use
 * the referrer's package context (mirroring the generator's imports) instead of a simple-name map
 * that collapses to whichever definition is last. An unresolved `RecursiveRef` must error like an
 * unresolved `ReferenceType`, not silently pass.
 */
class RefResolutionSpec extends AnyFlatSpec with Matchers {
  ...
  private val template = MultiTemplate("com.example", List(
    TemplateDefinition("A", ObjectType(ListMap("child" -> FieldDef(ReferenceType("B"), optional = false))), Some("p1")),
  ...
  it should "report an unresolved recursive reference instead of silently passing" in {
    val t = MultiTemplate("com.example", List(
      TemplateDefinition("N", ObjectType(ListMap("next" -> FieldDef(RecursiveRef("Ghost"), optional = false))))))
  ...
  it should "still validate a well-formed self-recursive structure" in {
    val tree = MultiTemplate("com.example", List(
