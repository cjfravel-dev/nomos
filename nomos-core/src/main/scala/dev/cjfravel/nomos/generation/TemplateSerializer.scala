...
/**
 * Serializes MultiTemplate and related model objects into Scala source code that can be embedded in generated files to
 * reconstruct the template at runtime.
 */
object TemplateSerializer {
  ...
  def serializeTemplateType(templateType: TemplateType): String =
    templateType match {
      ...
      case EnumType(enumName, values) =>
        s"""EnumType("${escapeString(enumName)}", List(${values
            .map(v => s""""${escapeString(v)}"""")
            .mkString(", ")}))"""
   ...
   */
  private def serializeAdditional(a: AdditionalProperties): String =
    a match {
