...
class EnumSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "parser" should "parse a scalar string enum" in {
    val d =
      parse("""{"name":"N","template":{"priority":{"type":"string","enum":["low","high"]}}}""").value.definitions.head
    d.templateType.asInstanceOf[ObjectType].fields("priority").fieldType shouldBe StringType(
      List(Enum(List("low", "high"))))
  }
  ...
  def multi: MultiTemplate =
    MultiTemplate(
      ...
      List(
        TemplateDefinition(
          "N",
          ObjectType(ListMap("p" -> FieldDef(StringType(List(Enum(List("low", "high")))), false))))))
  ...
  "generator" should "keep String type for enum" in {
    new CodeGenerator(GeneratorConfig("com.example", "target/test-gen"))
      ...
      .get
      .content should include("p: String")
  }
