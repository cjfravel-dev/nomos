...
/**
 * Two fields in the same package can each declare an inline enum with the same name. The generator emits one file per
 * enum, keyed by output path. When two same-named enums in a package carry different value sets they are genuinely
 * different types, so silently keeping one produces a decoder that rejects the other's valid JSON. These specs pin that
 * such a clash is a hard error, while identical enums still collapse to one file and same-named enums in distinct
 * packages stand.
 */
