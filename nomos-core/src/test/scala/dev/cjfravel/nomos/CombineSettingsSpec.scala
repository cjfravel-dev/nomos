...
class CombineSettingsSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "combine" should "allow the same non-default value in more than one file" in {
    val merged =
      MultiTemplate.combine(List(t("com.example.a", listType = "Array"), t("com.example.b", listType = "Array"))).value
    merged.listType shouldBe "Array"
  }

  "combine" should "fail when two files give a setting conflicting non-default values" in {
    val result =
      MultiTemplate.combine(List(t("com.example.a", listType = "Array"), t("com.example.b", listType = "Vector")))
      ...
      MultiTemplate
        .combine(
          List(
            ...
            t("com.example.b", listType = "Vector", dateType = "java.time.Instant")))
        .left
        .get
    msg should include("listType")
