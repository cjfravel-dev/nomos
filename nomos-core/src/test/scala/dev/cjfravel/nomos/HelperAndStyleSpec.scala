...
class FromJsonStyleSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "default" should "generate an Either-returning fromJson" in {
    val t =
      parser.parseMultiTemplate("""{"definitions":[{"name":"U","template":{"id":"string"}}]}""", "com.example").value
    val content =
      new CodeGenerator(GeneratorConfig("com.example", "target/test-gen"))
        .generateMulti(t)
        .value
        .find(_.fileName == "U.scala")
        .get
        .content
  ...
  "throwing style" should "generate a throwing fromJson" in {
    val t =
      parser
        .parseMultiTemplate(
          """{"fromJsonStyle":"throwing","definitions":[{"name":"U","template":{"id":"string"}}]}""",
          "com.example")
        .value
    t.fromJsonStyle shouldBe "throwing"
    val content =
      new CodeGenerator(GeneratorConfig("com.example", "target/test-gen", throwingFromJson = true))
        .generateMulti(t)
        .value
        .find(_.fileName == "U.scala")
        .get
