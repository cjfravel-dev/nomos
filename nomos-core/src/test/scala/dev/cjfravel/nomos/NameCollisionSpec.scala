...
/**
 * toPascalCase splits on '-' and '_', so distinct source values can normalize to one identifier and
 * emit duplicate case objects/classes (#11); and a definition and an inline enum sharing a name in
 * one package map to the same output file, silently clobbering one (#32). Both are uncompilable or
 * silently-wrong outcomes, so generation rejects them up front with an actionable message.
 */
class NameCollisionSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "generateMulti" should "reject enum values that normalize to the same case-object name" in {
    val d = TemplateDefinition("E", ObjectType(ListMap("s" -> FieldDef(EnumType("S", List("a-b", "a_b")), optional = false))))
  ...
  it should "reject variant keys that normalize to the same class name" in {
    val disc = TypeDiscriminator("kind",
      ...
      ), ListMap.empty, includeInOutput = true)
  ...
  it should "reject a definition and an inline enum that map to the same output file" in {
    val t = MultiTemplate("com.example", List(
      ...
      TemplateDefinition("Widget", ObjectType(ListMap("shade" -> FieldDef(EnumType("Color", List("red", "blue")), optional = false))))
    ))
  ...
  it should "still generate distinct enum values without collision" in {
    val d = TemplateDefinition("E", ObjectType(ListMap("s" -> FieldDef(EnumType("S", List("active", "inactive")), optional = false))))
  ...
  it should "still allow intentional variantNames grouping (same class name for several keys)" in {
    val disc = TypeDiscriminator("kind",
      ...
      ), ListMap.empty, includeInOutput = true, variantNames = Map("circle" -> "Round", "round" -> "Round"))
