...
class DefaultsSpec extends AnyFlatSpec with Matchers with EitherValues with CompileHarness {
  ...
  "validator" should "not report a defaulted field as missing when it is absent" in {
    val t = parse("""{"name":"N","template":{"id":"string","exclude_from_ad":{"type":"boolean","default":false}}}""").value
  ...
  it should "not report a defaulted common or variant field as missing in a discriminated union" in {
    val t = parse(
      """{"name":"U","template":{"$type":{"discriminator":"type",
  ...
  "generated code with a defaulted field" should "decode the default when absent and validate" in {
    val tmpl = """{"definitions":[{"name":"N","template":{"id":"string","exclude_from_ad":{"type":"boolean","default":false}}}]}"""
    val driver =
      """package com.example
        |object DefDriver {
        |  def run(): String = {
        ...
        |}
        |""".stripMargin
    val files = GeneratedFile("com/example/DefDriver.scala", driver) ::
      new CodeGenerator(GeneratorConfig("com.example", "target/test-gen"))
        .generateMulti(parser.parseMultiTemplate(tmpl, "com.example").value).value
