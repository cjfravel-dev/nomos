...
/**
 * Phase two (custom validators) is skipped for a definition whose reachable subtree declares no validators, so a
 * validator-free template pays no second traversal. Reachability is subtree-scoped: an unrelated definition's
 * validators do not force the walk. Correctness (which validators run) is unchanged and covered by
 * ValidatorContextSpec.
 */
class ValidatorPruningSpec extends AnyFlatSpec with Matchers with BeforeAndAfterEach {
  ...
  "a definition with its own top-level validator" should "run phase two" in {
    val t =
      MultiTemplate("com.example", List(TemplateDefinition("Parent", obj("id" -> strField), validators = List("v"))))
    new MultiValidator(t).reachesAnyValidator("Parent") shouldBe true
  }
  ...
  "pruning" should "not change which validators run (recursive type, deep)" in {
    ...
    ValidatorRegistry.register("node.ok") { ctx =>
      if (ctx.node.asObject.flatMap(_.field("v")).flatMap(_.asString).contains("bad"))
        List(ValidationError(ctx.path, "bad node", "ok", "bad"))
      else Nil
    }
    val t =
      MultiTemplate(
        ...
        List(
          TemplateDefinition(
            "Node",
            obj("v" -> strField, "next" -> FieldDef(RecursiveRef("Node"), optional = true)),
            validators = List("node.ok"))))
  ...
  "a validator on a $ref inside a union branch" should "reach and run only for the matched branch" in {
    ValidatorRegistry.register("w.ok") { ctx =>
      if (ctx.node.asObject.flatMap(_.field("w")).flatMap(_.asString).contains("bad"))
        List(ValidationError(ctx.path, "bad w", "ok", "bad"))
      else Nil
    }
    val t =
      MultiTemplate(
        ...
        List(
          TemplateDefinition(
            "Holder",
            obj("u" -> FieldDef(UnionType(List(StringType(), ReferenceType("WithV"))), optional = false))),
