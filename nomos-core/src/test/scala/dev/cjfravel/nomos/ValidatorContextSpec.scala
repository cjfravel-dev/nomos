...
import org.scalatest.{EitherValues, OptionValues, BeforeAndAfterEach}
import scala.collection.immutable.ListMap

/**
 * Custom validators receive a ValidatorContext (the node at their definition's level, the whole
 * document root, and the JSON path), and they run at every level a definition appears — including
 * definitions reached through a $ref — not only the top-level definition being validated.
 */
class ValidatorContextSpec extends AnyFlatSpec with Matchers with EitherValues with OptionValues with BeforeAndAfterEach with CompileHarness {
  ...
  private def parentChild = MultiTemplate("com.example", List(
    TemplateDefinition("Parent", ObjectType(ListMap(
      "child" -> FieldDef(ReferenceType("Child"), optional = false)))),
    TemplateDefinition("Child", ObjectType(ListMap(
  ...
  "a top-level validator" should "receive a context whose node equals the root and path is 'root'" in {
    ...
    val m = MultiTemplate("com.example", List(TemplateDefinition("R",
      ObjectType(ListMap("id" -> FieldDef(StringType(), false))), validators = List("cap"))))
  ...
  "re-entrant validation" should "not disturb the outer call's root" in {
    ...
    val reentrant = MultiTemplate("com.example", List(
      TemplateDefinition("Parent", ObjectType(ListMap(
        "child" -> FieldDef(ReferenceType("Child"), optional = false)))),
      TemplateDefinition("Child", ObjectType(ListMap(
        "value" -> FieldDef(StringType(), optional = false))),
        validators = List("child.checkRoot")),
      TemplateDefinition("Other", ObjectType(ListMap(
  ...
  "a nested validator" should "not run when a sibling subtree is structurally invalid" in {
    ...
    val withSibling = MultiTemplate("com.example", List(
      TemplateDefinition("Parent", ObjectType(ListMap(
        "child" -> FieldDef(ReferenceType("Child"), optional = false),
    ...
    new MultiValidator(withSibling).validate("""{"child":{"value":"x"},"count":"nope"}""", "Parent") shouldBe a[Left[_, _]]
    ran shouldBe false
  }
  ...
  "a $ref'd validator inside an array" should "run for each element with an indexed path" in {
    ...
    val withArray = MultiTemplate("com.example", List(
      TemplateDefinition("Parent", ObjectType(ListMap(
        "children" -> FieldDef(ArrayType(ReferenceType("Child")), optional = false)))),
      TemplateDefinition("Child", ObjectType(ListMap(
        "value" -> FieldDef(StringType(), optional = false))),
        validators = List("child.valueOk"))))
    new MultiValidator(withArray).validate("""{"children":[{"value":"a"},{"value":"b"}]}""", "Parent") shouldBe a[Right[_, _]]
