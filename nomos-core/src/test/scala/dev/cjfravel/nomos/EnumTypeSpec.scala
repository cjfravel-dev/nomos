...
class EnumTypeSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  val tmpl = """{"name":"User","template":{"status":{"type":"string","enum":["active","inactive"],"as":"enumType","name":"Status"}}}"""

  "parser" should "produce an EnumType when as=enumType" in {
    val d = parse(tmpl).value.definitions.head
    d.templateType.asInstanceOf[ObjectType].fields("status").fieldType shouldBe EnumType("Status", List("active", "inactive"))
  }
  ...
  "generator" should "type the field as the enum and emit a sealed trait file" in {
    val files = new CodeGenerator(GeneratorConfig("com.example", "target/test-gen")).generateMulti(parse(tmpl).value).value
  ...
  "validator" should "accept enum values and reject others" in {
    val t = MultiTemplate("com.example", List(TemplateDefinition("User",
