...
trait CompileHarness {
  ...
  protected def compile(files: List[GeneratedFile], additionalClasspath: Seq[String] = Nil): (Seq[String], String) = {
    ...
    settings.classpath.value =
      (System.getProperty("java.class.path") +: additionalClasspath).mkString(java.io.File.pathSeparator)
   ...
   */
  protected def runDriver(
      files: List[GeneratedFile],
      driverFqn: String,
      additionalClasspath: Seq[String] = Nil): String = {
    val (errs, outDir) = compile(files, additionalClasspath)
    // scalastyle:off throwerror
    if (errs.nonEmpty) throw new AssertionError("compilation failed:\n" + errs.mkString("\n"))
    // scalastyle:on throwerror
    val urls = (outDir +: additionalClasspath).map(path => new java.io.File(path).toURI.toURL).toArray
