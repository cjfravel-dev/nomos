    ...
    val t =
      MultiTemplate(
        ...
        List(
          TemplateDefinition(
            "N",
            ObjectType(ListMap("d" -> FieldDef(DateType(), false), "ts" -> FieldDef(DateTimeType(), false))))))
  ...
  "datetime validation" should "match the decoder for the configured dateTimeType" in {
    ...
    val local =
      new MultiValidator(
        MultiTemplate(
          "com.example",
          List(TemplateDefinition("N", ObjectType(ListMap("ts" -> FieldDef(DateTimeType(), false)))))))
