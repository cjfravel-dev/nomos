...
/**
 * A present JSON `null` must be accepted by validation for fields that decode treats leniently —
 * optional, nullable, or defaulted — rather than reported as a type mismatch. (Previously the
 * validator had no null handling, so a nullable field carrying `null` failed.)
 */
class NullableValidationSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "a present null" should "be accepted for an optional+nullable field" in {
    val v = validator("""{"definitions":[{"name":"N","template":{
  ...
  it should "be accepted for nullable common and variant fields of a discriminated union" in {
    val v = validator("""{"definitions":[{"name":"U","template":{"$type":{"discriminator":"type",
