...
class FileWriterSpec extends AnyFlatSpec with Matchers with BeforeAndAfterEach {
  ...
  it should "not follow directory symlinks while pruning generated files" in {
    val externalDir = new File("target/test-file-writer-external")
    val generated = new File(externalDir, "External.scala")
    externalDir.mkdirs()
    java.nio.file.Files.write(
      generated.toPath,
      "// Code generated by Nomos. DO NOT EDIT.\nobject External\n".getBytes(java.nio.charset.StandardCharsets.UTF_8))
    testOutputDir.mkdirs()
    val link = new File(testOutputDir, "linked")

    try {
      java.nio.file.Files.createSymbolicLink(link.toPath, externalDir.getCanonicalFile.toPath)
      fileWriter.pruneOrphans(testOutputDir, Set.empty)
      generated should exist
    } finally {
