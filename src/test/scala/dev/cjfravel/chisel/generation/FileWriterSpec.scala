...
class FileWriterSpec extends AnyFlatSpec with Matchers with BeforeAndAfterEach {
  ...
  "FileWriter" should "write a single file successfully" in {
    val generatedFile: GeneratedFile = GeneratedFile(
  ...
  it should "overwrite existing files" in {
    val generatedFile1 = GeneratedFile(
      relativePath = "Test.scala",
      content = "// Version 1\n"
    )
    
    val generatedFile2 = GeneratedFile(
      relativePath = "Test.scala",
      content = "// Version 2\n"
    )
  ...
  it should "return all errors when writing multiple files fails" in {
    ...
    val files = List(
      GeneratedFile("com/example/User.scala", "content"),
      GeneratedFile("com/example/Order.scala", "content")
    )
  ...
  it should "generate a successful write report when all files write" in {
    val files = List(
      GeneratedFile("File1.scala", "content 1"),
      GeneratedFile("File2.scala", "content 2")
    )
  ...
  it should "provide detailed error messages" in {
    val generatedFile = GeneratedFile(
      relativePath = "com/example/Test.scala",
      content = "content"
    )
